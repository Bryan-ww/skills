...
import { getLargeFileCacheUrl } from '../../utils/cacheLargeFile'
...
export class Resource extends EventEmitter {
  ...
  loadItem(item) {
    return new Promise(async (resolve, reject) => {
      ...
      let path = item.path
      if (item.useCache) {
        const cacheUrl = await getLargeFileCacheUrl(item.path)
        if (cacheUrl) {
          path = cacheUrl
        }
        console.log('cacheUrl', item.name, cacheUrl)
      }
      this.loaders[item.type].load(
        path,
        (data) => {
          this.itemsLoaded++
          this.emit('onProgress', path, this.itemsLoaded, this.itemsTotal)
          resolve({ ...item, data })
        },
        null,
        (err) => {
