...
export class Label3d {
  constructor({ scene, camera, time, sizes, canvas, id }) {
    this.scene = scene
    this.camera = camera
    this.time = time
    this.sizes = sizes
    ...
    let css3dRender = new CSS3DRenderer({ antialias: true }) // 实例化css3d渲染器
    ...
    css3dRender.domElement.id = this.id
  ...
  create(content = '', className = '', isSprite = false) {
    ...
    label.isHide = () => {
      return label.element.style.visibility === 'hidden'
    }
    /**
     * 隐藏
     */
  ...
  destroy() {
    if (this.css3dRender) {
      let label3d = document.querySelector('#' + this.id)
      if (!label3d || !label3d.parentNode) {
        return
      }
      label3d.parentNode.removeChild(this.css3dRender.domElement)
