<template>
  <div v-if="show" class="layer-tree-select" :style="{ 'width': width }">
    <div class="top">
      <div class="top__search bigscreen-card-common-bg">
        <input class="top__input" v-model="searchText" :placeholder="props.placeholder" @keyup.enter="inputSearch()"></input>
    ...
    <div class="flex mt-[0.1rem] gap-[0.1rem] h-[5.3rem]">
      ...
      <div class="result-box bigscreen-card-common-bg">
        ...
        <div v-show="!loading" class="result-scroll">
          <el-collapse v-model="deviceStore.collapseActiveId" accordion>
            <el-collapse-item v-for="item1 in list" :key="item1.id" :title="getCollapseTitle(item1)" :name="item1.id">
              <div class="pl-[0.2rem]">
                <div v-for="(item2, index) of item1.children" :key="index" class="result-item" @click="toTarget(item2)">
                  <div class="result-item__left">
                    {{ item2.name }}
                  </div>
                  <div class="result-item__right" :class="{
                    'error-color': item2.runningState.value === '3',
                    'online-color': item2.runningState.value === '2',
                    'offline-color': item2.runningState.value === '1',
                  }">
                    {{ item2.runningState.display }}
                  </div>
                </div>
              </div>
            </el-collapse-item>
...
<script setup>
  ...
  const getCollapseTitle = (item) => {
    return `${item.label}（${item.children?.length || 0}）`
  }
...
.layer-tree-select {
  ...
  :deep(.el-collapse) {
    --el-collapse-header-font-size: 0.16rem;
  }
