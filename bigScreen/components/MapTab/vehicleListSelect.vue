<template>
  <div v-if="isShow" class="layer-tree-select" :style="{ 'width': width }">
    <div class="top">
    ...
    <div class="flex mt-[0.1rem] gap-[0.1rem] h-[5.3rem]">
      ...
      <div class="result-box bigscreen-card-common-bg">
        ...
        <div v-show="!loading" class="result-scroll">
          <div v-for="item in list" :key="item.id" class="result-item" @click="toTarget(item)">
            <div class="result-item__left">{{ item.name }}</div>
            <div class="result-item__right">{{ item.stateCn }}</div>
          </div>
        </div>
      </div>
